...
public class MainApp extends Application {
    ...
    private AllocationEngine engine = new AllocationEngine();
    ...
    private VBox buildSidebar() {
        ...
        sb.setPrefWidth(210);
        sb.setStyle(
                "-fx-background-color:" + BG_SIDEBAR + "; -fx-border-color:" + BORDER + "; -fx-border-width:0 1 0 0;");
        ...
        exit.setStyle("-fx-background-color:" + DANGER
                + "; -fx-text-fill:white; -fx-font-weight:bold; -fx-font-size:13px; -fx-padding:10 20 10 20; -fx-background-radius:6; -fx-cursor:hand;");
    ...
    private Button navBtn(String text, Runnable action) {
        ...
        String normal = "-fx-background-color:transparent; -fx-text-fill:" + TEXT_MUTED
                + "; -fx-font-size:13px; -fx-cursor:hand; -fx-background-radius:6;";
        String hover = "-fx-background-color:" + BG_CARD + "; -fx-text-fill:" + TEXT_PRIMARY
                + "; -fx-font-size:13px; -fx-cursor:hand; -fx-background-radius:6;";
        btn.setStyle(normal);
    ...
    private VBox card(String title) {
        ...
        c.setStyle("-fx-background-color:" + BG_CARD + "; -fx-background-radius:10; -fx-border-color:" + BORDER
    ...
    private TextField input(String prompt) {
        ...
        tf.setStyle("-fx-background-color:" + BG_INPUT + "; -fx-text-fill:" + TEXT_PRIMARY + "; -fx-prompt-text-fill:"
                + TEXT_MUTED + "; -fx-border-color:" + BORDER
    ...
    private void ok(Label l, String msg) {
        l.setStyle("-fx-text-fill:" + SUCCESS + "; -fx-font-size:12px;");
        l.setText(msg);
    }

    private void err(Label l, String msg) {
        l.setStyle("-fx-text-fill:" + DANGER + "; -fx-font-size:12px;");
        l.setText(msg);
    }
    ...
    if (header != null) {
        ...
        tv.setRowFactory(rv -> {
            ...
            row.selectedProperty().addListener((obs, was, is) -> {
                if (is)
                    row.setStyle("-fx-background-color:" + ACCENT + ";");
                else
                    row.setStyle("-fx-background-color:" + BG_INPUT + ";");
            });
            return row;
...
private <T> TableColumn<T, Object> col(String title, String prop, double w) {
    ...
    private void showDashboard() {
        ...
        show(buildPage("Dashboard", stats, info));
    }
    ...
    private VBox statCard(String label, String value, String color) {
        ...
        c.setPadding(new Insets(20));
        c.setPrefWidth(180);
        c.setStyle("-fx-background-color:" + BG_CARD + "; -fx-background-radius:10; -fx-border-color:" + color
                + "; -fx-border-radius:10; -fx-border-width:1;");
    ...
    private void showStudents() {
        ...
        TextField idF = input("Student ID");
        ...
        addBtn.setOnAction(e -> {
            if (idF.getText().trim().isEmpty() || nmF.getText().trim().isEmpty()) {
                err(msg, "ID and Name are required.");
                return;
            }
    ...
    private void showListings() {
        ...
        TextField linkF = input("Download Link");
        ...
        typeBox.setStyle("-fx-background-color:" + BG_INPUT + "; -fx-text-fill:" + TEXT_PRIMARY + "; -fx-border-color:"
                + BORDER + "; -fx-border-radius:6; -fx-background-radius:6;");
        ...
        addBtn.setOnAction(e -> {
            ...
            if (lt != Resource.ListingType.DIGITAL) {
                try {
                    res.setAskingPrice(Double.parseDouble(priceF.getText().trim()));
                } catch (Exception ex) {
                }
            } else {
        ...
        form.getChildren().addAll(
                lbl("Resource ID"), ridF,
                lbl("Name"), rnF,
                lbl("Type"), typeBox,
                lbl("Owner"), ownerF,
                lbl("Price (Rs.)"), priceF,
                lbl("Drive Link (Digital only)"), linkF,
                addBtn, msg);
    ...
    private void showRequests() {
        ...
        bBtn.setOnAction(e -> {
            ...
            try {
                ...
                if (ok) {
                    ok(bMsg, "Buy request posted.");
                    bn.clear();
                    br.clear();
                    bb.clear();
                    showRequests();
                } else {
                    err(bMsg, "Student not found. Register first.");
                }
            } catch (NumberFormatException ex) {
                err(bMsg, "Invalid budget.");
            }
        });
        buyCard.getChildren().addAll(lbl("Your Name"), bn, lbl("Resource Wanted"), br, lbl("Budget (Rs.)"), bb, bBtn,
                bMsg);
        ...
        rBtn.setOnAction(e -> {
            if (rn.getText().trim().isEmpty() || rr.getText().trim().isEmpty()
                    || ro.getText().trim().isEmpty() || rs.getText().trim().isEmpty()
                    || re.getText().trim().isEmpty()) {
                err(rMsg, "All fields required.");
                return;
            }
            try {
                boolean ok = engine.addRequest(new Request(
                ...
                if (ok) {
                    ...
                    err(rMsg, "Student not found. Register first.");
                }
            } catch (NumberFormatException ex) {
                err(rMsg, "Invalid number.");
            }
        });
        ...
        HBox.setHgrow(tableCard, Priority.ALWAYS);
        show(buildPage("Requests", layout));
    }
    ...
    private void showRentMatching() {
        ...
        Label info = new Label(
                "Matches RENT requests with available listings.\nHigher offering price = higher priority. Conflicts detected automatically.");
        info.setStyle("-fx-text-fill:" + TEXT_MUTED + "; -fx-font-size:13px;");
        info.setWrapText(true);

        TextArea out = outputBox(SUCCESS);
        out.setText("Press 'Run Rent Matching' to see results...");

        Button runBtn = btn("Run Rent Matching", SUCCESS);
        runBtn.setOnAction(e -> {
            List<Request> results = engine.runRentAllocation();
            if (results.isEmpty()) {
                out.setText("No rent requests to process.");
                return;
            }
    ...
    private void showExchangeCycle() {
        ...
        Label info = new Label(
                "Detects if two or more students can swap resources directly.\nIf cycle found, contacts are revealed so they can meet and swap — no money needed.");
        info.setStyle("-fx-text-fill:" + TEXT_MUTED + "; -fx-font-size:13px;");
        info.setWrapText(true);

        TextArea out = outputBox(ACCENT);
        out.setText("Press 'Detect Exchange Cycle' to run analysis...");
    ...
    private void showDigital() {
        ...
        if (digital.isEmpty()) {
            ...
            for (Resource r : digital) {
                ...
                Label owner = new Label("By: " + r.getOwnedBy());
                owner.setStyle("-fx-text-fill:" + TEXT_MUTED + "; -fx-font-size:12px;");
                info.getChildren().addAll(name, owner);

                Label link = new Label(r.getDownloadLink() != null ? r.getDownloadLink() : "No link");
        ...
        Label note = new Label("Free access — copy the link and open in browser.");
